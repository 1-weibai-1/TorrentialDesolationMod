...
package wei_bai.torrential_desolation.world.biome;

import wei_bai.torrential_desolation.block.AuragrassblockBlock;
import wei_bai.torrential_desolation.block.AuraWoodTianyuncongBlock;
import wei_bai.torrential_desolation.block.AuraLeavestianyuncongBlock;
...
public class FairytreeFormation1Biome extends TorrentialDesolationModElements.ModElement {
	public static Biome biome;

	public FairytreeFormation1Biome(TorrentialDesolationModElements instance) {
		super(instance, 64);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(936181759).setWaterColor(936181759).setWaterFogColor(936181759)
						.withSkyColor(936181759).withFoliageColor(936181759).withGrassColor(936181759).build();
				BiomeGenerationSettings.Builder biomeGenerationSettings = new BiomeGenerationSettings.Builder()
						.withSurfaceBuilder(SurfaceBuilder.DEFAULT.func_242929_a(new SurfaceBuilderConfig(AuragrassblockBlock.block.getDefaultState(),
								AuraDirtBlock.block.getDefaultState(), AuraDirtBlock.block.getDefaultState())));
				biomeGenerationSettings.withFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.TREE
						.withConfiguration(
								(new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(AuraWoodTianyuncongBlock.block.getDefaultState()),
										new SimpleBlockStateProvider(AuraLeavestianyuncongBlock.block.getDefaultState()),
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.FOREST).depth(0.1f).scale(0.6f)
						.temperature(0.5f).downfall(0.5f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("torrential_desolation:fairytree_formation_1"));
			}
		}
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("fairytree_formation_1_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("fairytree_formation_1_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("fairytree_formation_1_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
