...
public class ConfigHandler {
	...
	public ConfigHandler() {
		...
		for (final Attribute attribute : ForgeRegistries.ATTRIBUTES) {
			...
			if (attribute instanceof RangedAttribute) {
				...
				TorrentialDesolationMod.LOGGER.debug("Skipping attribute {}, not a ranged attribute.", attribute.getRegistryName());
			}
		}
	...
	public void applyChanges () {
		...
		for (final Entry<RangedAttribute, AttributeData> entry : this.attributes.entrySet()) {
			...
			if (data.enabled.get()) {
				...
				TorrentialDesolationMod.LOGGER.debug("Changing range for {} from {}-{} to {}-{}", ranged.getRegistryName(), FORMAT.format(ranged.minimumValue), FORMAT.format(ranged.maximumValue), FORMAT.format(data.min.get()), FORMAT.format(data.max.get()));
				...
				TorrentialDesolationMod.LOGGER.debug("Skipping attribute {}. Disabled by config.", ranged.getRegistryName());
			}
		}
	}
	...
	private static class AttributeData {
		...
		public AttributeData(RangedAttribute attribute, ForgeConfigSpec.Builder builder) {
			...
			final String group = id.getNamespace().replace(".", "_") + "_" + id.getPath().replace(".", "_");
			...
			this.max = builder.defineInRange("max", Math.max(attribute.maximumValue, 1.7976931348623157E308d), -Double.MAX_VALUE, Double.MAX_VALUE);
